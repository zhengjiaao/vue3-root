...
import {DrawEvent, DrawCircle} from '@antv/l7-draw';
...
onMounted(() => {
  if (mapContainer.value) {
    ...
    // 绘制圆形
    scene.on('loaded', () => {
      const drawCircle = new DrawCircle(scene, { distanceOptions: {},});
      drawCircle.enable();

      drawCircle.on(DrawEvent.Change, (allFeatures: GeoJSONFeature[]) => {
