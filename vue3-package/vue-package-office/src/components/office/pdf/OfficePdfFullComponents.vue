  ...
  <div class="pdf-reader">
    ...
    <div class="viewer-container">
      <div class="sidebar" :class="{ 'collapsed': sidebarCollapsed }">
        ...
        <div class="thumbnail-list" v-if="!sidebarCollapsed">
          <div
...
// 手动定义 VueOfficePdfRef 类型
// interface VueOfficePdfRef {
...
const pdfData = ref<ArrayBuffer | string | null>(null);
...
const applyScalePreset = () => {
  if (scalePreset.value === 'auto') {
    scale.value = 1;
  } else if (scalePreset.value === 'width') {
    scale.value = 1;
    // 这里需要根据容器宽度计算适合的缩放比例
    // 实际实现可能需要获取PDF页面宽度和容器宽度
  } else {
